$(document).ready(function(){
	...
	$('#pic').on('change',function(){
		var pic = $(this).val();
		if(pic.length > 0) {
			$("#confirmPic").html("<img src='img/check.jpg' class = 'img1'>");
		} else {
			$("#confirmPic").html("<p class = 'icon-remove' style = 'color:red'>&nbsp;&nbsp;Choose an Image</p>");
		}	
	});

	$('#validate').on('click',function(){		
		if ($('#pic').val().length > 0 && $('#fname').val().length >=2 && $('#lname').val().length >= 1 && $('#checkedEmail').val() == "T" && $('#password').val().length >= 8 && $('#password').val() ==  $('#confirm').val()) {
			return true;
		} else {
			alert("Fill in All");
			return false;
